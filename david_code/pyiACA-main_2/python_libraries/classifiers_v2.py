...
# Machine Learning Libraries (TensorFlow is optional — only needed when NN=True)
try:
...
def build_knn_classifier(
    ...
    if metric == "cosine" and algorithm == "auto":
        algorithm = "brute"
...
def train_ACA_model(X_AC_train, y_train, FAKE_AC, NN, knn_kwargs=None):
    if NN:
        clf_AC = create_model()
        ...
        clf_AC = build_knn_classifier(**knn_kwargs)
