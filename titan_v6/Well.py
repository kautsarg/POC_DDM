...
class Well:
    ...
    def well_temp_mean_then_lin(self):
        # v06 path: firmware streams a single per-frame `tem_lin` value in the readout payload tail
        # (Lacewing_Thread.py:2876, decoded into readout_meta["temp_avg_Vs_shifted"] - 100).
        # When that override is set on the Well by `titan_load_and_preprocessing`, prefer it: the
        # spatial temperature pixels are zero in v06 captures and would otherwise produce a NaN trace.
        override = getattr(self, "well_temp_1d_v06", None)
        if override is not None:
            t = np.asarray(override, dtype=float)
            if t.size == 0:
                return t
            t0 = t[0]
            if not np.isfinite(t0):
                # Fall back to the first finite sample as the baseline so we still get a usable trace.
                finite = t[np.isfinite(t)]
                t0 = float(finite[0]) if finite.size else 0.0
            return t - t0
    ...
    @property
    def well_2d_nl_bs_active_mean_filt(self):
        """Filtered version of well_2d_nl_bs_active_mean using moving average filter"""
        filt_order = 60  # Default filter order
        return lfilter(b=np.ones(filt_order) / filt_order, a=[1], x=self.well_2d_nl_bs_active_mean)
    ...
    @property
    def well_3d_start(self):
        """Linearized data windowed from pre-heat start to end."""
        return self.well_3d_lin[:, :, self.idx_start:self.idx_end]

    @property
    def well_2d_start(self):
        n_time = self.well_3d_start.shape[2]
        return self.well_3d_start.reshape(-1, n_time, order='C').T

    @property
    def well_2d_start_bs(self):
        return self.well_2d_start - self.well_2d_start[0, :]

    @property
    def well_2d_start_bs_active(self):
        return self.well_2d_start_bs[:, self.idx_active]

    @property
    def well_2d_start_bs_active_mean(self):
        return np.mean(self.well_2d_start_bs_active, axis=1)

    def well_2d_bs_filt(self, filt_order=60):
        return lfilter(b=np.ones(filt_order) / filt_order, a=[1], x=self.well_2d_bs.T).T

    def well_2d_bs_active_filt(self, filt_order=60):
        return lfilter(b=np.ones(filt_order) / filt_order, a=[1], x=self.well_2d_bs_active.T).T

    def well_2d_bs_active_mean_filt(self, filt_order=60):
