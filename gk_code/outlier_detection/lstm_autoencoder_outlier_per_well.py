...
def build_lstm_autoencoder(timesteps):
    """Builds an LSTM Autoencoder with Dropout to prevent memorizing outliers."""
...
def run_lstm_autoencoder_per_well_pipeline(dataset_names, dataset_curves, Y_well, ref_curves, ae_plot_path, threshold_percentiles=["elbow", 90, 95], epochs=60, batch_size=128, save_plot=True):
    ...
    for i, (name, curves) in enumerate(zip(dataset_names, dataset_curves)):
        ...
        print(f"  -> Running Per-Well LSTM Autoencoder Filtering for {clean_title}...")

        full_mse = np.full(len(Y_well), -1.0)
        
        results_filter = {"lstm_ae_mse": full_mse}
        for pct in threshold_percentiles:
            results_filter[f"lstm_ae_label_{pct}"] = np.full(len(Y_well), -1)
        ...
        for well in unique_wells:
            ...
            # Early stopping prevents over-training on the small ~1000 sample per-well dataset
        ...
        filtered_dict = {k: v for k, v in results_filter.items() if k != "lstm_ae_mse"}
        ...
        if save_plot:
            for pct in threshold_percentiles:
                ...
                if str(pct).lower() == "elbow":
                    ...
                    sub_text = f"Per-Well LSTM | Kept bottom {pct}% of MSE reconstruction errors per Well"
                    
                html = init_html_report(title=f"Per-Well LSTM Outliers: {clean_title}", subtitle=sub_text)
                ...
                for well in unique_wells:
                    ...
                    if str(pct).lower() == "elbow" and well in elbow_data:
                        ...
                        ax_elbow.set_title(f"Well {well} - Specialized LSTM Error Distribution", fontweight="bold")
                ...
                html += "</div></body></html>"
                save_path = os.path.join(ae_plot_path, f"{name}_per_well_lstm_ae_{pct}.html")
