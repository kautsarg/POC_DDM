...
def run_autoencoder_pipeline(dataset_names, dataset_curves, Y_well, ref_curves, ae_plot_path, threshold_percentiles=["elbow", 90, 95], epochs=50, batch_size=256, save_plot=True):
    ...
    for i, (name, curves) in enumerate(zip(dataset_names, dataset_curves)):
        ...
        print(f"  -> Running Autoencoder Filtering for {clean_title}...")
        ...
        # 2. Train the Autoencoder (ONLY ONCE PER DATASET)
        ...
        full_mse[valid_mask] = mse
        
        # 4. Initialize results dictionary for ALL thresholds
        results_filter = {"ae_mse": full_mse}
        for pct in threshold_percentiles:
            results_filter[f"ae_label_{pct}"] = np.full(len(Y_well), -1)
            
        elbow_data = {}
        
        # 5. Apply all Thresholds per Class (Well)
        for well in unique_wells:
            ...
            if len(valid_well_mse) > 0:
                ...
                for pct in threshold_percentiles:
                    ...
                    keep_in_class_mask = (well_mse <= cutoff_threshold) & (well_mse != -1.0)
                    global_keep_indices = well_indices[keep_in_class_mask]
                    results_filter[label_col][global_keep_indices] = 1

        # Build DataFrame (stripping out the raw ae_mse scores)
        filtered_dict = {k: v for k, v in results_filter.items() if k != "ae_mse"}
        ...
        if save_plot:
            for pct in threshold_percentiles:
                label_col = f"ae_label_{pct}"
                if str(pct).lower() == "elbow":
                    sub_text = "Threshold dynamically set using the Knee/Elbow Method"
                else:
                    sub_text = f"Kept bottom {pct}% of MSE reconstruction errors per Well"
                    
                html = init_html_report(title=f"Autoencoder Outliers: {clean_title}", subtitle=sub_text)
                ...
                for well in unique_wells:
                    ...
                    if str(pct).lower() == "elbow" and well in elbow_data:
                        ...
                        ax_elbow.set_title(f"Well {well} - Autoencoder Error Distribution", fontweight="bold")
                ...
                html += "</div></body></html>"
                save_path = os.path.join(ae_plot_path, f"{name}_ae_{pct}.html")
