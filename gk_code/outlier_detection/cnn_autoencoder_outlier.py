...
def run_cnn_autoencoder_pipeline(dataset_names, dataset_curves, Y_well, ref_curves, ae_plot_path, threshold_percentiles=["elbow", 90, 95], epochs=60, batch_size=128, save_plot=True, downsample_factor=1, per_well=True):
    ...
    for i, (name, curves) in enumerate(zip(dataset_names, dataset_curves)):
        ...
        print(f"  -> Running {method_str.upper()} CNN Autoencoder (DS:{downsample_factor}x) for {clean_title}...")
        ...
        if save_plot:
            for pct in threshold_percentiles:
                ...
                if str(pct).lower() == "elbow":
                    sub_text = f"[{method_name} CNN | DS: {downsample_factor}x] Threshold dynamically set using Knee/Elbow Method"
                else:
                    sub_text = f"[{method_name} CNN | DS: {downsample_factor}x] Kept bottom {pct}% of MSE reconstruction errors"
                    
                html = init_html_report(title=f"CNN Autoencoder Outliers: {clean_title}", subtitle=sub_text)
                ...
                for well in unique_wells:
                    ...
                    if str(pct).lower() == "elbow" and well in elbow_data:
                        ...
                        ax_elbow.set_title(f"Well {well} - {method_name} CNN Error Distribution", fontweight="bold")
