...
def plot_melt_curves(df, ylim, filename, NMETA=0):
...
def plot_standard_curves(df, filename, NMETA=0):
    if(NMETA==0):
        df_ = df.transpose().copy()
    else:
...
def plot_melt_peak_distributions(df, filename):
    ...
    for i, (target, df_) in enumerate(df.groupby('Target')):  
        ...
        if i==8:
            ax.text(df_['MeltPeaks'].quantile(q=0.2), np.max(y)+1.0, f"{target} ({df_['MeltPeaks'].mean():.2f})", color=color, fontsize=12, weight='bold',
                    horizontalalignment='center', verticalalignment='center')
        elif i==1:
            ax.text(df_['MeltPeaks'].quantile(q=0.5), np.max(y)+1.2, f"{target} ({df_['MeltPeaks'].mean():.2f})", color=color, fontsize=12, weight='bold',
                    horizontalalignment='center', verticalalignment='center')
            
        else:
            ax.text(df_['MeltPeaks'].quantile(), np.max(y)+0.8, f"{target} ({df_['MeltPeaks'].mean():.2f})", color=color, fontsize=12, weight='bold',
                    horizontalalignment='center', verticalalignment='center') 
...
def plot_amplification_curves(df, ylim, NMETA=0, target_column="well", filename=""):
    n_target = len(np.unique(df[target_column].values))
    fig, ax = plt.subplots(1, n_target, figsize=(15, 2), dpi=300, constrained_layout=True)
    ax = ax.flatten()
    ...
    for i, (target, df_t) in enumerate(df.groupby(target_column)):
        ...
        if(NMETA > 0):
            curves = df_t.iloc[:, NMETA:].transpose()
        else:
            curves = df_t.transpose()
