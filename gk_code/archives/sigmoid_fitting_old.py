...
def sigmoid_2p(x, b, c):
    """2-Parameter Logistic (2PL)
    y = 1 / (1 + exp(-b * (x - c)))
    Parameters: b (slope), c (center)
    """
    return 1.0 / (1.0 + np.exp(-b * (x - c)))


def sigmoid_3p(x, a, b, c):
    """3-Parameter Logistic (3PL)
    y = a + (1 - a) / (1 + exp(-b * (x - c)))
    Parameters: a (bottom), b (slope), c (center)
    """
    return a + (1.0 - a) / (1.0 + np.exp(-b * (x - c)))


def sigmoid_4p(x, a, d, b, c):
    """4-Parameter Logistic (4PL) - Most common
    y = a + (d - a) / (1 + exp(-b * (x - c)))
    Parameters: a (bottom), d (top), b (slope), c (center)
    """
    return a + (d - a) / (1.0 + np.exp(-b * (x - c)))
...
def sigmoid_gompertz(x, a, b, c):
    """Gompertz Function
    y = a * exp(-b * exp(-c * x))
    Parameters: a (max), b (decay), c (rate)
    """
    return a * np.exp(-b * np.exp(-c * x))


def sigmoid_weibull(x, a, b, c, d):
    """Weibull Function
    y = c + (d - c) * (1 - exp(-(a * x)^b))
    Parameters: a (scale), b (shape), c (min), d (max)
    """
    return c + (d - c) * (1.0 - np.exp(-(a * x)**b))


def sigmoid_hill(x, vmax, km, n):
    """Hill Equation (Pharmacology)
    y = vmax * x^n / (km^n + x^n)
    Parameters: vmax (max response), km (EC50), n (Hill coefficient)
    """
    return vmax * x**n / (km**n + x**n)


def sigmoid_hill_norm(x, ec50, n):
    """Normalized Hill Equation [0 to 1]
    y = x^n / (ec50^n + x^n)
    Parameters: ec50 (EC50), n (Hill coefficient)
    """
    return x**n / (ec50**n + x**n)


def sigmoid_boltzmann(x, a, b, c, d):
    """Boltzmann Function (Electrophysiology)
    y = a + (b - a) / (1 + exp((c - x) / d))
    Parameters: a (bottom), b (top), c (midpoint), d (slope)
    """
    return a + (b - a) / (1.0 + np.exp((c - x) / d))


def sigmoid_exponential(x, a, b, c):
    """Exponential Sigmoid
    y = a * (1 - exp(-b * x)) + c
    Parameters: a (amplitude), b (rate), c (offset)
    """
...
def extract_kinetic_parameters(x, y, params, threshold=0.1):
    ...
    F_max = np.max(sigmoid_5p(x, *params))
    F_min = np.min(sigmoid_5p(x, *params))
    threshold_20pct = F_min + 0.2 * (F_max - F_min)  # 20% of range above minimum
    ...
    # At x = 0: y = y0 - m*x0
    Cy0 = y_inflection - dy_inflection * x_inflection
...
def extract_kinetic_parameters_original(x, y, threshold=0.1, deriv_method='gradient'):
    ...
    F_max = np.max(y)
    F_min = np.min(y)
    threshold_20pct = F_min + 0.2 * (F_max - F_min)
    
    above_20pct = y > threshold_20pct
    crossing_indices = np.where(above_20pct)[0]
    ...
    if not np.isnan(xp1):
        x_inflection = xp1
        y_inflection = y[np.argmin(np.abs(x - x_inflection))]
        dy_inflection = dy_dx_vals[np.argmin(np.abs(x - x_inflection))]
        Cy0 = y_inflection - dy_inflection * x_inflection
    else:
        Cy0 = np.nan
...
def _denormalize_params(params, func_name, x_min, x_max, y_min, y_max):
    ...
    if func_name == "2PL":
        # sigmoid_2p(x, b, c): y = 1 / (1 + exp(-b * (x - c)))
        # Normalized: y_norm = 1 / (1 + exp(-b_norm * (x_norm - c_norm)))
        # Need to denormalize: c, b stays the same (slope in normalized space)
        b, c = params
        c_denorm = c * (x_max - x_min) + x_min
        return np.array([b / (x_max - x_min + 1e-10), c_denorm])
    
    elif func_name == "3PL":
        # sigmoid_3p(x, a, b, c): y = a + (1 - a) / (1 + exp(-b * (x - c)))
        # Denormalize: a (amplitude), b (slope), c (center)
        a, b, c = params
        a_denorm = a * (y_max - y_min) + y_min
        c_denorm = c * (x_max - x_min) + x_min
        b_denorm = b / (x_max - x_min + 1e-10)
        return np.array([a_denorm, b_denorm, c_denorm])
    
    elif func_name == "4PL":
        # sigmoid_4p(x, a, d, b, c): y = a + (d - a) / (1 + exp(-b * (x - c)))
        # Denormalize: a (bottom), d (top), b (slope), c (center)
        a, d, b, c = params
        a_denorm = a * (y_max - y_min) + y_min
        d_denorm = d * (y_max - y_min) + y_min
        c_denorm = c * (x_max - x_min) + x_min
        b_denorm = b / (x_max - x_min + 1e-10)
        return np.array([a_denorm, d_denorm, b_denorm, c_denorm])
        ...
        # sigmoid_gompertz(x, a, b, c): y = a * exp(-b * exp(-c * x))
        # a is max response, denormalize; b and c stay same
        a, b, c = params
        a_denorm = a * (y_max - y_min) + y_min
        return np.array([a_denorm, b, c])
    
    elif func_name == "Weibull":
        # sigmoid_weibull(x, a, b, c, d): y = c + (d - c) * (1 - exp(-(a * x)^b))
        # Denormalize: a (scale), b (shape), c (min), d (max)
        a, b, c, d = params
        c_denorm = c * (y_max - y_min) + y_min
        d_denorm = d * (y_max - y_min) + y_min
        a_denorm = a / (x_max - x_min + 1e-10)
        return np.array([a_denorm, b, c_denorm, d_denorm])
    
    elif func_name == "Hill":
        # sigmoid_hill(x, vmax, km, n): y = vmax * x^n / (km^n + x^n)
        # vmax is max response, denormalize; km (EC50), n stays same
        vmax, km, n = params
        vmax_denorm = vmax * (y_max - y_min) + y_min
        km_denorm = km * (x_max - x_min) + x_min
        return np.array([vmax_denorm, km_denorm, n])
    
    elif func_name == "Hill_Norm":
        # sigmoid_hill_norm(x, ec50, n): y = x^n / (ec50^n + x^n)
        # ec50 needs denormalization
        ec50, n = params
        ec50_denorm = ec50 * (x_max - x_min) + x_min
        return np.array([ec50_denorm, n])
    
    elif func_name == "Boltzmann":
        # sigmoid_boltzmann(x, a, b, c, d): y = a + (b - a) / (1 + exp((c - x) / d))
        # Denormalize: a (bottom), b (top), c (midpoint), d (slope)
        a, b, c, d = params
        a_denorm = a * (y_max - y_min) + y_min
        b_denorm = b * (y_max - y_min) + y_min
        c_denorm = c * (x_max - x_min) + x_min
        d_denorm = d * (x_max - x_min) + 1e-10
        return np.array([a_denorm, b_denorm, c_denorm, d_denorm])
    
    elif func_name == "Exponential":
        # sigmoid_exponential(x, a, b, c): y = a * (1 - exp(-b * x)) + c
        # a (amplitude), b (rate), c (offset)
        a, b, c = params
        a_denorm = a * (y_max - y_min)
        c_denorm = c * (y_max - y_min) + y_min
        b_denorm = b  # rate stays same
        return np.array([a_denorm, b_denorm, c_denorm])
...
def fit_2p(x, y, p0=None, bounds=None, maxfev=100000, normalize=True):
    """Fit 2-Parameter Logistic (2PL)"""
    if p0 is None:
        p0 = (1.0, np.median(x))
    if bounds is None:
        bounds = [(0.01, 100), (np.min(x), np.max(x))]
    
    return _fit_sigmoid(sigmoid_2p, x, y, p0=p0, bounds=bounds, 
                       maxfev=maxfev, func_name="2PL")


def fit_3p(x, y, p0=None, bounds=None, maxfev=100000, normalize=True):
    """Fit 3-Parameter Logistic (3PL)"""
    if p0 is None:
        p0 = (np.min(y), 1.0, np.median(x))
    if bounds is None:
        bounds = [(-1e6, 1e6), (0.01, 100), (np.min(x), np.max(x))]
    
    return _fit_sigmoid(sigmoid_3p, x, y, p0=p0, bounds=bounds, 
                       maxfev=maxfev, func_name="3PL")


def fit_4p(x, y, p0=None, bounds=None, maxfev=100000, normalize=True):
    """Fit 4-Parameter Logistic (4PL)"""
    if p0 is None:
        if normalize:
            p0 = (0.0, 1.0, 10.0, 0.5) 
        else:
            p0 = (np.min(y), np.max(y), 1.0, np.median(x))
            
    if bounds is None:
        if normalize:
            bounds = [(-0.5, 1.5), (-0.5, 1.5), (0.01, 500), (-0.5, 1.5)]
        else:
            bounds = [(-1e6, 1e6), (-1e6, 1e6), (0.01, 100), (np.min(x), np.max(x))]
    
    return _fit_sigmoid(sigmoid_4p, x, y, p0=p0, bounds=bounds, 
                       maxfev=maxfev, normalize=normalize, func_name="4PL")
...
def fit_gompertz(x, y, p0=None, bounds=None, maxfev=100000, normalize=True):
    """Fit Gompertz Function"""
    if p0 is None:
        p0 = (np.max(y), 1.0, 0.1)
    if bounds is None:
        bounds = [(0.1, 1e6), (0.01, 100), (0.001, 10)]
    
    return _fit_sigmoid(sigmoid_gompertz, x, y, p0=p0, bounds=bounds, 
                       maxfev=maxfev, normalize=normalize, func_name="Gompertz")


def fit_weibull(x, y, p0=None, bounds=None, maxfev=100000, normalize=True):
    """Fit Weibull Function"""
    if p0 is None:
        p0 = (1.0, 2.0, np.min(y), np.max(y))
    if bounds is None:
        bounds = [(0.1, 100), (0.1, 10), (-1e6, 1e6), (-1e6, 1e6)]
    
    return _fit_sigmoid(sigmoid_weibull, x, y, p0=p0, bounds=bounds, 
                       maxfev=maxfev, normalize=normalize, func_name="Weibull")

def fit_hill(x, y, p0=None, bounds=None, maxfev=100000, normalize=True):
    """Fit Hill Equation"""
    if p0 is None:
        p0 = (np.max(y), np.median(x[x > 0]) if np.any(x > 0) else 1.0, 1.0)
    if bounds is None:
        bounds = [(0.1, 1e6), (0.01, 1e6), (0.1, 10)]
    
    return _fit_sigmoid(sigmoid_hill, x, y, p0=p0, bounds=bounds, 
                       maxfev=maxfev, normalize=normalize, func_name="Hill")


def fit_hill_norm(x, y, p0=None, bounds=None, maxfev=100000, normalize=True):
    """Fit Normalized Hill Equation [0 to 1]"""
    if p0 is None:
        p0 = (np.median(x[x > 0]) if np.any(x > 0) else 1.0, 1.0)
    if bounds is None:
        bounds = [(0.01, 1e6), (0.1, 10)]
    
    return _fit_sigmoid(sigmoid_hill_norm, x, y, p0=p0, bounds=bounds, 
                       maxfev=maxfev, normalize=normalize, func_name="Hill_Norm")


def fit_boltzmann(x, y, p0=None, bounds=None, maxfev=100000, normalize=True):
    """Fit Boltzmann Function"""
    if p0 is None:
        p0 = (np.min(y), np.max(y), np.median(x), 1.0)
    if bounds is None:
        bounds = [(-1e6, 1e6), (-1e6, 1e6), (np.min(x), np.max(x)), (0.01, 100)]
    
    return _fit_sigmoid(sigmoid_boltzmann, x, y, p0=p0, bounds=bounds, 
                       maxfev=maxfev, normalize=normalize, func_name="Boltzmann")


def fit_exponential(x, y, p0=None, bounds=None, maxfev=100000, normalize=True):
    """Fit Exponential Sigmoid"""
    if p0 is None:
        p0 = (np.max(y) - np.min(y), 0.1, np.min(y))
    if bounds is None:
        bounds = [(-1e6, 1e6), (0.001, 100), (-1e6, 1e6)]
    
    return _fit_sigmoid(sigmoid_exponential, x, y, p0=p0, bounds=bounds, 
                       maxfev=maxfev, normalize=normalize, func_name="Exponential")
