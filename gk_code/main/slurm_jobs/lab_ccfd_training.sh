#!/bin/bash
#SBATCH --job-name=lab_ccfd_training
...
# Initial RCFD: lightweight early encoder (cnn/gru/trans x cgd/ctd x SC0-3) = 24 models
for SC in 0 1 2 3; do
    SARG=(); [ "$SC" -gt 0 ] && SARG=(--supcon "$SC")
    run_train --condreg "${SARG[@]}"
done

# New RCFD + New CCFD: dual-backbone early branch (cgd/ctd x SC0-3) = 16 models
python -u /vol/bitbucket/gk225/POC_DDM/gk_code/main/03d_ccfd_training.py \
    --task_id $SLURM_ARRAY_TASK_ID \
    --exp_folder "$TRAIN_FOLDER" \
    --curve_type ori_curve \
    --n_splits 5
