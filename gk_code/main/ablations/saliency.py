...
def _compute_feature_sensitivity_profiles_all(X_batch, timestamps, feat_names,
    ...
    # Loop by sample (not by timestep) so each sample's unperturbed baseline fit
    # is computed once, not T_len times -- it doesn't depend on t_idx.
    with threadpool_limits(limits=1):
...
def plot_latent_feature_mapping(
    ...
    saliency profiles, and feat_matrix rows to this subset (e.g. one class's
    correctly-classified samples, mirroring plot_per_label_saliency_heatmap).
    feat_sensitivity is left as-is: it's a property of the feature extractor
    itself, computed once per dataset regardless of subset."""
...
def render_latent_feature_mapping_figure(
    ...
    for sec in sections:
        ...
        for i in range(k):
            ...
            # Bar height/colour normalised to this latent's own best match, so a
            # weakly-explained latent's relative ranking is still legible -- the
            # true (un-normalised) score is still what's printed in the label.
