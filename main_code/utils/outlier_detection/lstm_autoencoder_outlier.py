...
def build_lstm_autoencoder(timesteps):
    ...
    # reused as a pretrained feature extractor.
...
def run_lstm_autoencoder_pipeline(dataset_names, dataset_curves, Y_well, ref_curves, ae_plot_path,
                                   threshold_percentiles=None, epochs=60, batch_size=128,
                                   save_plot=True, downsample_factor=1, save_encoder_dir=None):
    ...
    base_label = f"lstm_ae_glb_ds{downsample_factor}"

    for i, (name, curves) in enumerate(zip(dataset_names, dataset_curves)):
        clean_title = name.replace("_", " ").title()
        print(f"  -> Running GLOBAL LSTM Autoencoder (DS:{downsample_factor}x) for {clean_title}...")
        ...
        if len(X_valid) >= 5:
            ...
            autoencoder.fit(X_scaled_3d, X_scaled_3d, epochs=epochs, batch_size=batch_size,
        ...
        if save_plot:
            for pct in threshold_percentiles:
                ...
                if str(pct).lower() == "elbow":
                    sub_text = f"[Global LSTM | DS: {downsample_factor}x] Threshold dynamically set using Knee/Elbow Method"
                else:
                    sub_text = f"[Global LSTM | DS: {downsample_factor}x] Kept bottom {pct}% of MSE reconstruction errors"
                ...
                for well in unique_wells:
                    ...
                    if str(pct).lower() == "elbow" and well in elbow_data:
                        ...
                        ax_elbow.set_title(f"Well {well} - Global LSTM Error Distribution", fontweight="bold")
